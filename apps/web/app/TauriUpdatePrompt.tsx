...
export function TauriUpdatePrompt() {
  ...
  return (
    <div className="tauri-update-backdrop" role="presentation">
      <section className="tauri-update-modal" role="dialog" aria-modal="true" aria-label="Homage update">
        {updater.state === "error" ? (
          <>
            <p className="tauri-update-eyebrow">업데이트 확인 실패</p>
            <h2>업데이트를 확인하지 못했습니다</h2>
            <p>{updater.error || "네트워크나 서명 검증 상태를 확인한 뒤 다시 시도하세요."}</p>
          ...
          <>
            <p className="tauri-update-eyebrow">데스크톱 업데이트</p>
            <h2>Homage {updater.version} 패치가 준비되었습니다</h2>
