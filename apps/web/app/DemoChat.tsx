    ...
    <section className="atanor-demochat">
      <div className="atanor-demochat-thread" ref={scrollRef}>
        {messages.length === 0 ? (
          <div className="atanor-demochat-empty">
            ...
            <h1>{language === "ko" ? "무엇이든 물어보세요" : "Ask me anything"}</h1>
            <p>{language === "ko" ? "로컬 그래프 추론 엔진이 근거를 갖춰 답하고, 모르면 정직하게 보류합니다." : "A local graph-reasoning engine — grounded answers, honest abstention."}</p>
      ...
      <form className="atanor-demochat-composer" onSubmit={send}>
        <input
          ...
          placeholder={language === "ko" ? "메시지를 입력하세요…" : "Message ATANOR…"}
          aria-label="message"
        />
        <button type="submit" disabled={busy || !input.trim()} aria-label="send">{busy ? "…" : "↑"}</button>
      </form>
      <div className="atanor-demochat-foot">{language === "ko" ? "로컬 엔진 · 외부 LLM 없음 · 출처를 갖춘 답변, 불확실하면 보류" : "Local engine · no external LLM · grounded, abstains when unsure"}</div>
    </section>
  );
}
