...
def brain_graph(
    ...
    max_edges: int = Query(default=3000, ge=1, le=10000),
) -> dict:
    layer_list = [part.strip() for part in layers.split(",") if part.strip()] if layers else None
    return aggregate_brain_graph(view=view, layers=layer_list, query=query, max_nodes=max_nodes, max_edges=max_edges)
